...
Rispondi SEMPRE in italiano. Usa valori tecnici precisi. Massima praticità operativa per un autista.`,
  ...
  // ── AUTOCOMPLETE NUMERI ──
  const numeriRegistrati = mezzi.map(m=>m.numero);
  const NumInput = ({val,onChange,placeholder}) => (
    <>
      <Inp list="num-list" placeholder={placeholder||"es. N082"} value={val}
        onChange={onChange} style={{fontFamily:"'IBM Plex Mono',monospace",letterSpacing:"0.08em"}}/>
      <datalist id="num-list">
        {numeriRegistrati.map(n=><option key={n} value={n}/>)}
      </datalist>
    </>
  );
  ...
  return (<>
    ...
    <div style={{background:C.bg,minHeight:"100vh",fontFamily:"'Rajdhani',sans-serif",color:C.text}}>
      ...
      {/* HEADER */}
      <div style={{background:C.bgCard,borderBottom:`2px solid ${C.accent}`,padding:"0 16px",
        display:"flex",alignItems:"center",gap:12,height:54,position:"sticky",top:0,zIndex:200}}>
